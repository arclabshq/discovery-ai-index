...
test("serves index.html directly for an app route before the asset layer can redirect it", async () => {
  const calls = [];
    ...
    }),
    {
      ASSETS: {
        fetch: async (request) => {
          ...
          return new Response(url.pathname === "/index.html" ? "app" : "redirected", {
            status: url.pathname === "/index.html" ? 200 : 307,
          });
        },
      },
    },
